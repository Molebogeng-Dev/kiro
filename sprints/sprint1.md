...
- **Config management** via environment variables (`python-decouple` or `django-environ`), with a `.env.example` committed and the real `.env` gitignored — no connection strings or keys hardcoded anywhere
...
4. Copy `.env.example` to `.env` and fill in your Supabase connection string
